...
export question=question-53
export folder=folder-53
...
sed -i '/^\s*name:/s/\(name:\s*\).*/\1question-53/' /home/student/.kube/config
kubectl config use-context $question  >> $LOGFILE 2>&1
kubectl config set-context --current --cluster $question --user kind-$question  >> $LOGFILE 2>53